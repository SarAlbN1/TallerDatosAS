...
public class VentaMDB {
    ...
    @KafkaListener(topics = "ventas-proveedor-b", groupId = "proveedor-b-group")
    public void procesarVenta(VentaCompletadaEvent event) {
        try {
            log.info("🏭 PROVEEDOR B - Venta recibida: Factura {} por ${}", 
            ...
            if (inventarioService.necesitaReposicion()) {
                log.info("📋 Generando orden de reposición para Proveedor B");
            ...
            log.info("✅ PROVEEDOR B - Venta procesada exitosamente: {}", event.getFacturaId());
            
        } catch (Exception e) {
            log.error("❌ PROVEEDOR B - Error procesando venta {}: {}", 
